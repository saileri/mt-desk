#!/usr/bin/env python3
"""MT Desk v7.1 — Insight-driven trading dashboard with advanced charts."""
...
def _fmt_hours(h):
    ...
    if h < 24:
        return f"{h:.1f}小時".rstrip("0").rstrip(".") + "小時"
    d = h / 24
    return f"{d:.1f}天".rstrip("0").rstrip(".") + "天"
...
def build_dashboard_html(account, trades, stats):
    ...
    cards = [
        ...
        ("盈虧比", f"{plr:.2f}", "平均盈利 ÷ 平均虧損，<1 表示每賺 1 元要賠更多元", "--muted" if plr < 1 else "--green"),
    ...
    card_html = "".join(
        f'<div class="kpi has-tip"><span class="lbl">{l}</span><span class="val" style="color:var({c})">{v}</span><span class="tip">{tip}</span></div>'
        for l, v, tip, c in cards
    )
  ...
  <div class="summary-card" style="grid-column:span 2">
    <div class="summary-title">品種偏好（交易次數佔比）<span class="hint">點擊品種可篩選明細</span></div>
    <div id="chart-symbol-pie" style="width:100%;height:300px"></div>
  </div>
  <div class="summary-card">
    <div class="summary-title">各品種盈虧貢獻</div>
    <div id="chart-symbol-pl" style="width:100%;height:300px"></div>
  </div>
</div>
<div class="summary-row compact">
  <div class="summary-card">
    <div class="summary-title">交易量佔比 <span class="hint">{total_volume_val:.2f} 手</span></div>
    <div id="chart-volume-donut" style="width:100%;height:180px"></div>
  </div>
  <div class="summary-card">
    <div class="summary-title">Swap / 利息 <span class="hint" style="color:{swap_color}">${total_swap_val:+,.2f}</span></div>
    <div id="chart-swap-bar" style="width:100%;height:180px"></div>
  </div>
  <div class="summary-card wide" style="justify-content:flex-start">
    <div class="summary-title">推薦洞察</div>
...
@media(max-width:600px){.summary-row,.summary-row.compact{grid-template-columns:1fr}.kpi-row{grid-template-columns:repeat(2,1fr)}}
@media print{@page{margin:12mm}.header,.date-bar,.toolbar,.theme-btn,.metric-guide{display:none!important}.chart-box{break-inside:avoid;page-break-inside:avoid}.table-wrap{overflow-x:visible}body{font-size:10pt;background:#fff!important;color:#000!important}.kpi{border:1px solid #ccc!important;background:#fff!important;box-shadow:none}.kpi .val{font-size:14pt}.kpi .lbl,.kpi .tip{color:#666!important}.chart-grid{grid-template-columns:1fr}.section-title{color:#000!important;border-bottom:1px solid #999}.summary-line{background:#f5f5f5!important;border:1px solid #ccc}.summary-card{background:#fff!important;border:1px solid #ccc}}
</style></head><body>
<div class="header"><h1>MT Desk v7.1 <span>交易習慣分析報表</span></h1>
...
function renderTable(){totalPages=Math.ceil(data.length/pageSize)||1;if(currentPage>totalPages)currentPage=totalPages;var start=(currentPage-1)*pageSize;var page=data.slice(start,start+pageSize);var h='';page.forEach(function(t){var cls=t.profit>0?'win':'loss';h+='<tr class="'+cls+'"><td>'+t.ticket+'</td><td>'+t.open+'</td><td>'+t.type+'</td><td>'+t.volume+'</td><td>'+t.symbol+'</td><td>'+(t.open_price||'-')+'</td><td>'+t.close+'</td><td>'+(t.close_price||'-')+'</td><td>'+(t.duration_str||'-')+'</td><td class="'+cls+'">$'+t.profit_per_lot.toFixed(2)+'</td><td>$'+(t.swap||0).toFixed(2)+'</td><td class="'+cls+'">$'+t.profit.toFixed(2)+'</td></tr>';});document.getElementById('tradeBody').innerHTML=h;var info=currentPage+'/'+totalPages+' ('+data.length+'筆)';document.getElementById('pageInfo').textContent=info;document.getElementById('pageInfo2').textContent=info;}
...
def main():
    root = tk.Tk()
    root.title("MT Desk v7.1")
    root.geometry("400x260")
    root.configure(bg="#f4f6f9")
    root.resizable(False, False)
    tk.Label(root, text="MT Desk v7.1", font=("Segoe UI", 22, "bold"), fg="#2563eb", bg="#f4f6f9").pack(pady=(24, 4))
